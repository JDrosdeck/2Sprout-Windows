#include "database.h"
#include "ui_database.h"
#include "mainwindow.h"

bool activateDB = false;

Database::Database(QWidget *parent) :
    QDialog(parent),
    m_ui(new Ui::Database)
{

    m_ui->setupUi(this);
        m_ui->lineEdit_9->setText(dbhost);
        m_ui->lineEdit_6->setText(dbuser);
        m_ui->lineEdit_7->setText(dbpass);
        m_ui->lineEdit_8->setText(dbdefault);
        m_ui->spinBox->setValue(dbport);
        if(dbtype == "none")
        {
            m_ui->comboBox_2->setCurrentIndex(0);
            m_ui->lineEdit_6->setEnabled(false);
            m_ui->lineEdit_7->setEnabled(false);
            m_ui->lineEdit_8->setEnabled(false);
            m_ui->lineEdit_9->setEnabled(false);
            m_ui->spinBox->setEnabled(false);
        }
        if(dbtype == "mysql")
            m_ui->comboBox_2->setCurrentIndex(1);
        if(dbtype == "postgres")
            m_ui->comboBox_2->setCurrentIndex(2);
}

Database::~Database()
{
    delete m_ui;
}

void Database::changeEvent(QEvent *e)
{
    switch (e->type()) {
    case QEvent::LanguageChange:
        m_ui->retranslateUi(this);
        break;
    default:
        break;
    }
}



void Database::on_pushButton_3_clicked()
{
    this->close();
}

void Database::on_pushButton_2_clicked()
{


    if(!m_ui->lineEdit_6->text().isEmpty() && !m_ui->lineEdit_7->text().isEmpty() && !m_ui->lineEdit_8->text().isEmpty() && !m_ui->lineEdit_9->text().isEmpty())
    {
        if(m_ui->comboBox_2->currentIndex() == 1)
        {
            dbtype = "mysql";
            useDatabase = true;
        }
        else if(m_ui->comboBox_2->currentIndex() == 2)
        {
            dbtype = "postgres";
            useDatabase = true;
        }
        else
        {
            dbtype = "none";
            useDatabase = false;
        }

        dbhost = m_ui->lineEdit_9->text();
        dbuser = m_ui->lineEdit_6->text();
        dbpass = m_ui->lineEdit_7->text();
        dbdefault = m_ui->lineEdit_8->text();
        dbport = atoi(m_ui->spinBox->text().toStdString().c_str());

        this->close();


    }
    else
    {
        if(m_ui->comboBox_2->currentIndex() != 0)
        {
            m_ui->label->clear();
            m_ui->label->setText("Invalid Configuration");
        }
        else
        {
            this->close();
        }
    }

}

void Database::on_comboBox_2_activated(int index)
{

    if(index == 0)
    {
        m_ui->lineEdit_6->setEnabled(false);
        m_ui->lineEdit_7->setEnabled(false);
        m_ui->lineEdit_8->setEnabled(false);
        m_ui->lineEdit_9->setEnabled(false);
        m_ui->spinBox->setEnabled(false);
    }
    else
    {

        m_ui->lineEdit_6->setEnabled(true);
        m_ui->lineEdit_7->setEnabled(true);
        m_ui->lineEdit_8->setEnabled(true);
        m_ui->lineEdit_9->setEnabled(true);
        m_ui->spinBox->setEnabled(true);
    }
}
