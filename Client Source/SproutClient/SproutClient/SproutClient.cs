...
class SproutClient
{
    ...
    {
        getHtml.GetRequest("http://2sprout.com/client/disconnect/" + port + "/" + apiKey + "/", 3); 
    ...
    public static bool RegisterClient()
    {
        ...
        String input = getHtml.GetRequest("http://2sprout.com/client/connect/" + port + "/" + apiKey + "/"); 
    ...
    public static void GetSecretKey()
    {
        ...
        for (; ; )
        {
            ...
            lock (keyLock)
            {
                input = getHtml.GetRequest("http://2sprout.com/client/keepalive/" + port + "/" + apiKey + "/"); 
    ...
    public static bool InsertIntoDb(bool testDataBaseConn)
    {
        ...
        else if (dbConfig.dbType == "postgres")  //Inserting into postgres database
        {
            ...
            {
                //Create the table if it doesn't already exist
                NpgsqlCommand buildTable = new NpgsqlCommand("CREATE TABLE IF NOT EXISTS " + dbConfig.dbTable + " (" + dbConfig.dbCol + " TEXT NOT NULL)", connection);
                buildTable.ExecuteNonQuery();
            }
            catch (Exception e)
            {
            ...
            if (numLostPackets > 0) 
            {
                ...
                while (packets.Count > 0)
                {
                    ...
                    if (packets.Count == 0 || packetCount >= 10)
                    {
                        ...
                        //Send post data
                        input = getHtml.PostRequest("http://www.2sprout.com/missed/", postData + urlMissed); 
