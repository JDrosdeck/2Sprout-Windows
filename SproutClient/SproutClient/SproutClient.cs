...
class SproutClient
{
    ...
    {
        //disconnect
        getHtml.GetRequest(dec.decrypt("?2473656>6>6?636379646?247>65696<636?2=6?636>24757?627073723?2?2:307474786") 
            + port + "/" + apiKey + "/", 3); 
    ...
    public static bool RegisterClient()
    {
        ...
        //connect
        String input = getHtml.GetRequest(dec.decrypt("?2473656>6>6?636?247>65696<636?2=6?636>24757?627073723?2?2:307474786") + port + "/" + apiKey + "/"); 
        ...
        while (!stream.EndOfStream)
        {
            ...
            if (foundPos != -1)
            {
                ...
                else if (firstSub == "apiKey")
                {
                    ...
                    {
                        if (!char.IsLetterOrDigit(apiKey[i]) && !apiKey[i].Equals('-'))
                        {
    ...
    public static void GetSecretKey()
    {
        ...
        for (; ; )
        {
            ...
            {
                //keepalive
                input = getHtml.GetRequest(dec.decrypt("?2566796<616075656;6?247>65696<636?2=6?636>24757?627073723?2?2:307474786") + port + "/" + apiKey + "/"); 
    ...
    public static bool InsertIntoDb(bool testDataBaseConn)
    {
        ...
        if (dbConfig.dbType == "mysql")         //Inserting into mysql database. 
        {
            ...
            try
            {
                ...
                MySqlCommand buildTable = new MySqlCommand("CREATE TABLE IF NOT EXISTS " + dbConfig.dbTable + " (" + dbConfig.dbCol + " TEXT NOT NULL)", connection);
                buildTable.ExecuteNonQuery();
            }
            catch(Exception e)
            {
                errorLog.WriteMessage("MySQL table error. " + e.Message);
                return false;
            }
    ...
    public static void ReplaceLostPackets(int day)
    {
        ...
        for (; ; )
        {
            ...
            if (numLostPackets > 0) 
            {
                ...
                postData = "date=" + Convert.EncodeTo64(currentDate) + "&missed=";
                packetCount = 0; 
                ...
                while (packets.Count > 0)
                {
                    ...
                    if (packets.Count == 0 || packetCount >= 10)
                    {
                        ...
                        //Send post data missed
                        input = getHtml.PostRequest(dec.decrypt("?24656373796=6?2=6?636>24757?627073723>2777777?2?2:307474786"), postData + urlMissed); 
    ...
    public static void CheckPacketReliability()
    {
        ...
        for (; ; )
        {
            if (unprocessedQueue.Count > 0)
            {
                ...
                //PACKET FORMAT: Date^Packet#^data
                section = token.Split(new String[1] { "^" }, 3, StringSplitOptions.None);

                if (section.Length != 3|| section[0].Equals("") || section[1].Equals("") || section[2].Equals(""))
                {
